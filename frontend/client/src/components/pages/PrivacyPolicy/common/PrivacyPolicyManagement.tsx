...
import { createPrivacyPolicy, updatePrivacyPolicy, deletePrivacyPolicyById, getAllPrivacyPolicy, PrivacyPolicy } from '../../../servicesApi/PrivacyPolicyApi';
...
export const PrivacyPolicyManagement: React.FC<PrivacyPolicyManagementProps> = ({ show, onHide, onRefresh }) => {
    const { user: currentUser } = useAuth();
    const [privacyPolicy, setPrivacyPolicy] = useState<PrivacyPolicy[]>([]);
    const [loading, setLoading] = useState(false);
    const [saving, setSaving] = useState(false);
    const [editingItem, setEditingItem] = useState<PrivacyPolicy | null>(null);
    ...
    const loadPrivacyPolicy = async () => {
        try {
            ...
            if (error.response?.data?.errors) {
                ...
                setErrorMessage('Не удалось загрузить политику конфиденциальности');
            }
            setTimeout(() => {
                setErrorMessage(null);
                setServerErrors({});
    ...
    const filteredItems = useMemo(() => {
        if (!searchTerm.trim()) return privacyPolicy;
        const term = searchTerm.toLowerCase().trim();
        return privacyPolicy.filter(item =>
            item.title.toLowerCase().includes(term) ||
            item.content.toLowerCase().includes(term)
        );
    }, [privacyPolicy, searchTerm]);
    ...
    const handleCreate = async () => {
        ...
        try {
            await createPrivacyPolicy(currentUser.id, {
                id: 0,
    ...
    const handleUpdate = async () => {
        ...
        try {
            await updatePrivacyPolicy(currentUser.id, {
                id: editingItem.id,
                title: formData.title.trim(),
                icon: formData.icon.trim() || '🔒',
                content: formData.content.trim(),
    ...
    // Удаление
    const handleDelete = async (item: PrivacyPolicy) => {
        if (!currentUser?.id) {
            setErrorMessage('Пользователь не авторизован');
            return;
        }
        ...
        try {
            await deletePrivacyPolicyById(currentUser.id, item.id);
    ...
    const handleEdit = (item: PrivacyPolicy) => {
        setEditingItem(item);
        setFormData({
            title: item.title,
            icon: item.icon || '🔒',
    ...
    return (
        <Modal show={show} onHide={onHide} size="xl" centered className={styles.modal}>
            <Modal.Header closeButton>
                <Modal.Title>🔒 Управление политикой конфиденциальности</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                {/* Уведомления */}
                {successMessage && (
                ...
                <Row>
                    ...
                    <Col md={5}>
                        <div className="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <Button size="sm" variant="outline-primary" onClick={loadPrivacyPolicy} disabled={loading}>
                                🔄 Обновить
                            </Button>
                        </div>
                        ...
                        <div className={styles.itemsList}>
                            {loading ? (
                                ...
                                <>
                                    <div className={styles.searchInfo}>Найдено: {filteredItems.length} из {privacyPolicy.length}</div>
